...
function moveElement(e) {
    ...
    if (dragging.el.tagName === 'A') {
        ...
        if (tgt.tagName === 'A')
            return tgt.insertAdjacentElement(position, dragging.el);
        if (nav.children.length === 0)
            return nav.prepend(dragging.el);
    }
    if (dragging.el.tagName === 'SECTION') {
        if (dragging.el.contains(tgt))
            return; // can't drop *inside* self
...
function extractDataFromDrop(e) {
    if (!dragging || !(dragging.el instanceof HTMLAnchorElement))
        return;
